...
export default async function ProjectPage({ searchParams }: SearchParamsProps) {
  ...
  const filteredProjects = tag ? allProjects.filter(project => project.tag.includes(tag)) : allProjects
  // sort by startDate|DESC
  const sortedProjects = filteredProjects.sort((a, b) => (new Date(a.startDate) < new Date(b.startDate) ? 1 : -1))
